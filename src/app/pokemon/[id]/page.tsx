...
function PokemonDetail() {
  ...
  const Weaknesses = getWeakness(
    pokemon.types.map((typeObj: any) => typeObj.type.name)
  );

  return (
    <div className={`flex flex-col items-center m-10 ${poppins.className}`}>
      <div className="absolute left-20 top-20">
        <button
          onClick={() => {
            router.push("/");
          }}
          className="bg-white p-5 rounded-full  hover:scale-115 transition-transform duration-300 shadow-lg"
        >
          Back
        </button>
      </div>
